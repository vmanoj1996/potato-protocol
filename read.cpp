...
// #include <boost/crc.hpp>  // For CRC computation

#include <cstring>
#include "testDataTransmission/packet.hpp"
...
int main() {
    ...
    try {
        ...
        boost::asio::serial_port serial(io, "/dev/ttyACM1"); // Change to your serial port
                ...
                if(memcmp(header, ref_packet.header, HEADER_LENGTH) == 0)
                {
                    ...
                    else
                    {
                        ...
                        if(crc_calculated == crc)
                        {
                            ...
                            // publish the packet to ROS
                            cout<<errorCount<<" "<<data.packetID<<" "<<timer.elapsed()<<endl;
